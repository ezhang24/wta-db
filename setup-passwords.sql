...
BEGIN
    DECLARE salt CHAR(8) DEFAULT NULL;
    DECLARE password_hash BINARY(64) DEFAULT NULL;
...
DELIMITER !
CREATE FUNCTION authenticate(user_val VARCHAR(20), password VARCHAR(20))
...
BEGIN
    ...
    SET salt_val = (SELECT salt FROM user_info WHERE username = user_val LIMIT 1);
    SET pass_hash = SHA2(CONCAT(salt_val, password), 256);

    SELECT COUNT(*) INTO valid
    FROM user_info
    WHERE username = user_val
...
CALL sp_add_user('bavilari', 'bea123');
CALL sp_add_user('elzhang', 'emily123');
CALL sp_add_user('admin', 'adminpw');
